...
export default class CartsManagerFs {
  ...
  addToCart = async (req) => {
    try {
      ...
      console.log(
        "🚀 ~ file: cartsManager.fs.js:81 ~ CartsManagerFs ~ addToCart= ~  users:",
        users
      );
      ...
      if (accion === "aumentar") {
        ...
        console.log(
          "🚀 ~ file: cartsManager.fs.js:101 ~ CartsManagerFs ~ addToCart= ~ addPcart:",
          addPcart
        );
  ...
  addCartsPg = async (idCart, idProductAddCart, userId) => {
    try {
      ...
      if (productExists) {
        ...
        const newProduct = {
          producto: Number(idProductAddCart),
